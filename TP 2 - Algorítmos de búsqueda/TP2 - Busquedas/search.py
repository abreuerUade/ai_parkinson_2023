...
def depthFirstSearch(problem):
    ...
    def recursiveDFS(currentState, directions, exploredNodes):
        ...
        for nextCoords, nextDirection, nextCost in successors:
            result = recursiveDFS(nextCoords, directions + [nextDirection], exploredNodes)
...
def breadthFirstSearch(problem):
    ...
    def recursiveBFS(nodes, exploredNodes):
        ...
        if currentState in exploredNodes:
            return recursiveBFS(nodes, exploredNodes) # Ya evalué este nodo
    ...
    def recursiveUCS(nodes, exploredNodes):
        ...
        if (currentState not in exploredNodes) or (currentCost < exploredNodes[currentState]):
            ...
            if problem.isGoalState(currentState):
                ...
                for nextCoords, nextDirection, nextCost in successors:
                    newNode = (nextCoords, directions + [nextDirection], currentCost + nextCost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    def recursiveAStar(nodes, exploredNodes):
        ...
        nodes.sort(key=lambda x: x[2] + heuristic(x[0], problem))  # Ordenar la lista por costo + heurística
        currentState, directions, currentCost = nodes.pop(0)

        if currentState in exploredNodes and currentCost >= exploredNodes[currentState]:
